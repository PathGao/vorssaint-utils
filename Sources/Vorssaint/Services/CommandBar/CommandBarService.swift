...
final class CommandBarService: ObservableObject {
    ...
    /// Each list's folded copy, kept until that list is assigned again. They
    /// land at different moments - the catalog up front, the apps, the windows,
    /// the menus and the selection as their background passes finish - which is
    /// why one opening indexes seven to ten times.
    ...
    private func index() {
        ...
        // The three maps are built from nothing every time. A row that left its
        // list - the menus of the app that was in front a moment ago - has to
        // disappear from them, or pressing it would act on an app nobody is
        // looking at.
        entriesByID = [:]
        normalizedByID = [:]
        entriesByStableKey = [:]
